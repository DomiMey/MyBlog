...
def register():
    ...
    if form.validate_on_submit():
        ...
        reg_password = generate_password_hash(form.password.data, method='pbkdf2:sha256' ,salt_length=8)
        reg_email = form.email.data
        if User.query.filter_by(name=reg_username).first():
            flash("Username already exists.")
            return render_template("register.html", form=form)
        if User.query.filter_by(email=reg_email).first():
            flash("Email already in use. Try logging in.")
            return redirect(url_for("login"))
        new_user = User(name=reg_username, password=reg_password, email=reg_email)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not User.query.filter_by(email=login_email).first():
